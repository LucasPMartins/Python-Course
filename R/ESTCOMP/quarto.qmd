...
```{r}
library(ggplot2)
ggplot(data = treino,mapping = (aes(x = body_mass_g, y = bill_length_mm, color= species))) + geom_point(alpha=0.5) + theme_minimal()
```
...
for( k in 1:nrow(treino)){
  ...
  classificacao[j] <- treino$species[order(distancias)[1]]
}
