...
# reconhecimento ao bom resultado alcançado durante o ano que passou. Para isto contratou
# você para desenvolver a aplicação que servirá como uma projeção de quanto será gasto
# com o pagamento deste abono.
...
while True:
    try:
        ...
        if salario == 0:
            break
        colaboradores.append((salario, salario * 0.2) if salario > 500 else (salario, 100))
...
for i,colab in enumerate(colaboradores):
    print(f'R$ {colab[0]:.2f}-{colab[1]:.2f}')
    abonos += colab[1]
    if colab[1] == 100:
        qtd_minimo += 1
    maior = colab[1] if colab[1] > maior else maior
